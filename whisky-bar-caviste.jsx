...
import { 
  ...
  ShoppingCart, Minus, Copy, PackageCheck
} from 'lucide-react';
...
const validateBackupData = (data) => {
  ...
  return [data.customWhiskies, data.stock, data.stockQty, data.order, data.favorites].some(v => v !== undefined);
};
      ...
      <div className="flex gap-0.5" aria-hidden="true">
        {[1, 2, 3, 4, 5].map((step) => (
          <div key={step} className={`h-1.5 w-3 rounded-full transition-all duration-300 ${
            numLevel >= step ? 'bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]' : 'bg-stone-800'
          }`} />
        ))}
      </div>
      <span className="text-[9px] uppercase tracking-widest text-stone-400 font-bold group-hover:text-amber-500 transition-colors">Tourbe</span>
    </div>
  );
};
  ...
  return (
    <div className="relative group w-full sm:w-auto">
      ...
      <input 
        ...
        value={value}
        onChange={onChange}
        className="bg-stone-900/80 border border-stone-700 text-amber-50 py-2.5 pl-10 pr-12 rounded text-sm focus-visible:outline-none focus-visible:border-amber-500 focus-visible:ring-2 focus-visible:ring-amber-500/40 w-full sm:w-64 transition-all placeholder:text-stone-400 font-serif"
      />
      <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-400 group-focus-within:text-amber-500 transition-colors" aria-hidden="true" />
      {value && (
  ...
  <button
    ...
    className={`flex flex-col items-center gap-3 p-5 rounded-sm transition-all duration-300 border group relative focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--whisky-bg)]
      ${active 
        ...
        : 'bg-stone-900/20 border-stone-800/50 hover:bg-stone-800/40 hover:border-stone-700'
      }`}
  >
    <span className={`text-2xl filter transition-transform duration-300 ${active ? 'scale-110 drop-shadow-[0_0_8px_rgba(245,158,11,0.4)]' : 'grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-90'}`}>
      {icon}
    </span>
  ...
  return (
    ...
    >
      ...
      <button 
        type="button"
        onClick={onClose} 
        className="fixed top-4 left-4 sm:top-8 sm:left-8 z-20 flex items-center gap-3 px-5 py-2.5 min-h-[44px] border border-stone-700 text-stone-300 hover:text-amber-400 hover:border-amber-700 bg-black/80 rounded-full text-xs font-bold uppercase tracking-widest transition-all"
      >
      ...
      <div className="relative z-10 max-w-4xl w-full text-center px-6 py-20 md:py-28">
        ...
        <div className="flex flex-wrap justify-center gap-6 mb-16 items-center">
          ...
          <div className="h-1 w-1 bg-stone-500 rounded-full" aria-hidden="true"></div>
          <span className="text-stone-400 font-serif text-xl">{whisky.region}</span>
          <div className="h-1 w-1 bg-stone-500 rounded-full" aria-hidden="true"></div>
        ...
        {whisky.tasting && (whisky.tasting.nose || whisky.tasting.palate || whisky.tasting.finish) && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border-t border-b border-stone-800 bg-stone-900/20 divide-y md:divide-y-0 md:divide-x divide-stone-800 mb-20">
...
const AddWhiskyForm = ({ onAdd, onCancel, initialWhisky = null }) => {
  ...
  const inputClass = "w-full px-4 py-3 bg-stone-900/80 border border-stone-700 text-amber-50 focus-visible:outline-none focus-visible:border-amber-500 focus-visible:ring-2 focus-visible:ring-amber-500/35 rounded transition-colors placeholder:text-stone-400 font-serif";
  ...
  const chipClass = (active) => `px-4 py-2 min-h-[44px] rounded-full text-[10px] uppercase tracking-wider font-bold border transition-all duration-300 inline-flex items-center gap-1.5 ${
    active
      ...
      : 'bg-transparent border-stone-700 text-stone-300 hover:border-stone-500 hover:text-stone-100'
  }`;
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-6 items-end">
        ...
        <div>
          ...
          <input id="whisky-color" name="color" type="color" value={form.color} onChange={e => setForm(p => ({ ...p, color: e.target.value }))} className="h-11 w-16 bg-stone-900/80 border border-stone-700 rounded cursor-pointer p-1" />
        </div>
      </div>
      ...
      <div className="flex gap-4 pt-6 border-t border-stone-800">
        <button type="button" onClick={onCancel} className="flex-1 py-3 border border-stone-700 text-stone-400 rounded hover:bg-stone-800 transition-colors uppercase text-xs tracking-widest font-bold">Annuler</button>
        <button type="submit" className="flex-1 py-3 bg-amber-700 text-stone-950 font-bold rounded hover:bg-amber-600 transition-colors uppercase text-xs tracking-widest shadow-lg shadow-amber-900/20">{initialWhisky ? 'Enregistrer' : 'Ajouter'}</button>
      </div>
    </form>
  );
};
        ...
        {whisky.reasons && whisky.reasons.length > 0 && (
          <div className="flex flex-wrap justify-center gap-2 mb-10">
            {whisky.reasons.map((reason, i) => (
              <span key={i} className="px-3 py-1 bg-stone-900 border border-stone-800 text-stone-400 text-[10px] uppercase tracking-wider rounded-full flex items-center gap-2">
...
const WhiskyCard = React.memo(({ whisky, isExpanded, onToggle, rank, onPromote, onFavorite, isFavorite, onServe }) => {
  ...
  return (
    <article 
      ref={cardRef}
      className={`bg-[var(--whisky-surface)] transition-all duration-500 overflow-hidden relative group border-t border-b sm:border border-stone-800/60
        ${isExpanded ? 'border-amber-900/50 bg-stone-950 shadow-2xl z-10 scale-[1.02] sm:rounded' : 'hover:bg-stone-900/30 sm:rounded hover:border-stone-700'}`}
    >
      <div className="p-6 md:p-8">
        ...
        <div className={`grid transition-all duration-500 ease-in-out ${isExpanded ? 'grid-rows-[1fr] opacity-100 mt-6 pt-6 border-t border-stone-800/50' : 'grid-rows-[0fr] opacity-0 mt-0 pt-0'}`}>
          <div className="overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-sm">
              <div className="space-y-1">
                ...
                <p className="text-stone-400 font-serif">{whisky.tasting?.nose || "—"}</p>
              </div>
              <div className="space-y-1">
                <span className="text-amber-500 text-[10px] uppercase tracking-widest font-bold">Bouche</span>
                <p className="text-stone-400 font-serif">{whisky.tasting?.palate || "—"}</p>
              </div>
              <div className="space-y-1">
                <span className="text-amber-500 text-[10px] uppercase tracking-widest font-bold">Finale</span>
                <p className="text-stone-400 font-serif">{whisky.tasting?.finish || "—"}</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap justify-end gap-3 mt-8">
              ...
              <button 
                ...
                className="flex items-center justify-center gap-2 px-3 py-3 border border-stone-700 rounded hover:border-red-500/70 hover:bg-red-950/20 text-xs uppercase tracking-widest transition-colors text-stone-300 hover:text-red-400 min-w-[44px] min-h-[44px]"
              >
      ...
      {!isExpanded && (
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 opacity-60 group-hover:opacity-100 transition-opacity text-stone-400" aria-hidden="true">
          <ChevronDown size={16} />
        </div>
      )}
    </article>
    ...
    <div className="min-h-screen text-[var(--whisky-body)] font-sans selection:bg-amber-900/50 selection:text-white">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-8 md:py-16">
        ...
        <header className="flex flex-col items-center mb-16 text-center">
          <div className="mb-8" aria-hidden="true">
            <img
              ...
              width="512"
              height="512"
              className="w-28 h-28 md:w-36 md:h-36 rounded-[22%] border border-amber-800/40 shadow-[0_15px_45px_-12px_rgba(0,0,0,0.9),0_0_30px_-5px_rgba(212,175,55,0.15)]"
            />
          </div>
        ...
        <nav className="flex justify-center mb-16" aria-label="Vue principale">
          <div className="inline-flex bg-stone-900/80 p-1.5 rounded-full border border-stone-700 shadow-lg">
            <button
              type="button"
              aria-pressed={view === 'home'}
              onClick={() => setView('home')}
              className={`flex items-center gap-2 px-6 sm:px-8 py-3 min-h-[44px] rounded-full text-xs font-bold uppercase tracking-widest transition-all duration-300 ${view === 'home' ? 'bg-[var(--whisky-gold)] text-black shadow-[0_0_20px_rgba(212,175,55,0.25)]' : 'text-stone-300 hover:text-stone-100 hover:bg-white/5'}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-6 sm:px-8 py-3 min-h-[44px] rounded-full text-xs font-bold uppercase tracking-widest transition-all duration-300 ${view === 'collection' ? 'bg-[var(--whisky-gold)] text-black shadow-[0_0_20px_rgba(212,175,55,0.25)]' : 'text-stone-300 hover:text-stone-100 hover:bg-white/5'}`}
            >
            ...
            <button
              ...
              className={`relative flex items-center gap-2 px-6 sm:px-8 py-3 min-h-[44px] rounded-full text-xs font-bold uppercase tracking-widest transition-all duration-300 ${view === 'commande' ? 'bg-[var(--whisky-gold)] text-black shadow-[0_0_20px_rgba(212,175,55,0.25)]' : 'text-stone-300 hover:text-stone-100 hover:bg-white/5'}`}
            >
        ...
        <main className="animate-fadeIn min-h-[500px]">
          {view === 'home' && (
            <div className="space-y-16">
              {totalOwned === 0 ? (
                <div className="text-center py-20 border border-dashed border-stone-700 rounded bg-stone-900/40">
                  <Wine size={48} className="mx-auto text-stone-400 mb-6" strokeWidth={1} aria-hidden="true" />
                      ...
                      <div className="flex flex-wrap justify-center gap-4 sm:gap-6 mb-6">
                        {['tourbé', 'fumé'].map(id => {
                          ...
                          return (
                            ...
                            >
                              <Flame size={18} className={selectedProfiles.includes(id) ? 'fill-amber-500 text-amber-500' : 'text-stone-400 group-hover:text-stone-300'} aria-hidden="true" />
                        ...
                        {recommendedWhiskies.length > 0 ? (
                          <div className="space-y-16">
                            ...
                            {recommendedWhiskies.length > 1 && (
                              <div className="max-w-4xl mx-auto pt-16 border-t border-stone-800">
                          ...
                          <div className="text-center py-20">
                            <p className="text-stone-400 font-serif text-xl italic mb-4">"Il n'y a pas de mauvais whisky. Il n'y a que des whiskies qui ne sont pas assez bons."</p>
                      ...
                      showBrowseMode ? (
                        ...
                        <div className="text-center py-12 flex flex-col items-center gap-8">
                          ...
                          <div className="flex flex-col sm:flex-row gap-6">
                            <button type="button" onClick={handleSurpriseMe} className="group relative px-8 py-4 min-h-[48px] bg-stone-900 border border-stone-700 hover:border-amber-700 rounded overflow-hidden transition-all">
                            ...
                            <button type="button" onClick={handleBrowseMode} className="group px-8 py-4 min-h-[48px] border border-stone-700 hover:bg-stone-900 rounded transition-all">
          ...
          {view === 'collection' && (
            <div className="space-y-8 animate-fadeIn">
              {/* Toolbar */}
              <div className="bg-stone-900/90 p-6 rounded-lg border border-stone-700 sticky top-4 z-30 shadow-2xl">
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
                    ...
                    <div className="flex gap-2 w-full sm:w-auto">
                      <div className="relative flex-1 sm:flex-none">
                        ...
                        <ArrowUpDown size={12} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 pointer-events-none" aria-hidden="true" />
                      </div>
                      
                      <button 
                        type="button"
                        onClick={() => setShowAddModal(true)} 
                        className="flex items-center justify-center gap-2 px-6 py-2.5 min-h-[44px] bg-amber-600 hover:bg-amber-500 text-black rounded text-xs font-bold uppercase tracking-widest transition-all shadow-lg shadow-amber-900/20 whitespace-nowrap"
                      >
                ...
                {sortedCollection.map(whisky => (
                    <article 
                      ...
                      className={`group flex items-center gap-4 p-4 md:p-5 rounded border transition-all duration-300 relative overflow-hidden
                        ${whisky.owned 
                          ? 'bg-[var(--whisky-panel)] border-stone-800 hover:border-stone-700' 
                          : 'bg-stone-950/30 border-stone-800 opacity-80 hover:opacity-100 grayscale hover:grayscale-0'}`}
                    >
                      <div className="shrink-0 flex items-center bg-stone-900/60 rounded-full border border-stone-800" title={whisky.owned ? "En stock" : "En rupture"}>
                        <button
                          type="button"
                    ...
                    <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                      ...
                      <div className="hidden md:flex items-center gap-2">
                        ...
                        ) : <span className="text-[10px] text-stone-400 uppercase tracking-widest">Non tourbé</span>}
                      </div>
              ...
              {sortedCollection.length === 0 && (
                <div className="text-center py-24 text-stone-300 bg-stone-900/20 border border-stone-700 rounded">
            ...
            <div className="space-y-12 animate-fadeIn max-w-3xl mx-auto">
              ...
              <section aria-labelledby="bon-heading">
                ...
                {orderEntries.length === 0 ? (
                  <div className="text-center py-16 border border-dashed border-stone-700 rounded bg-stone-900/40">
                    <ShoppingCart size={40} className="mx-auto text-stone-400 mb-6" strokeWidth={1} aria-hidden="true" />
                    <p className="text-stone-300 font-serif italic text-lg mb-2">Le bon de commande est vide.</p>
                    <p className="text-stone-400 text-sm max-w-md mx-auto">Ajoutez des bouteilles depuis l'inventaire (icône panier) ou depuis les ruptures ci-dessus.</p>
                  </div>
                ) : (
                  <div className="space-y-8">
                    <div className="grid grid-cols-1 gap-2">
                      {orderEntries.map(({ whisky, quantity }) => (
                        <div key={whisky.id} className="flex items-center justify-between gap-4 p-4 rounded border border-stone-800 bg-[var(--whisky-panel)]">
                          <div className="flex items-center gap-3 min-w-0">
                            <ColorBadge color={whisky.color} />
                            <div className="min-w-0">
                              <p className="font-serif text-lg text-stone-100 truncate">{whisky.name}</p>
                          ...
                          <div className="shrink-0 flex items-center gap-1">
                            <div className="flex items-center bg-stone-900/60 rounded-full border border-stone-800">
                    ...
                    <div className="flex flex-wrap justify-center gap-3">
                      <button type="button" onClick={handleCopyOrder} className="flex items-center gap-2 px-5 py-3 min-h-[44px] border border-stone-700 text-stone-300 hover:text-amber-300 hover:border-amber-700 rounded text-xs font-bold uppercase tracking-widest transition-colors">
                        <Copy size={14} aria-hidden="true" /> Copier
                      </button>
                      <button type="button" onClick={handleShareOrder} className="flex items-center gap-2 px-5 py-3 min-h-[44px] border border-stone-700 text-stone-300 hover:text-amber-300 hover:border-amber-700 rounded text-xs font-bold uppercase tracking-widest transition-colors">
                        <Share2 size={14} aria-hidden="true" /> Partager
                      </button>
                      <button type="button" onClick={handleMailOrder} className="flex items-center gap-2 px-5 py-3 min-h-[44px] border border-stone-700 text-stone-300 hover:text-amber-300 hover:border-amber-700 rounded text-xs font-bold uppercase tracking-widest transition-colors">
                    ...
                    <div className="text-center pt-6 border-t border-stone-800">
                      <button
                        ...
                        className="inline-flex items-center gap-3 px-10 py-4 min-h-[48px] bg-amber-700 text-stone-950 font-bold rounded hover:bg-amber-600 transition-colors uppercase text-xs tracking-widest shadow-lg shadow-amber-900/20"
                      >
                        <PackageCheck size={18} aria-hidden="true" /> Commande reçue — mettre le stock à jour
                      </button>
                      <p className="text-stone-400 text-xs mt-4 max-w-md mx-auto">Ajoute les quantités commandées au stock de la cave et vide le bon de commande.</p>
                    </div>
                  </div>
                )}
              </section>
        ...
        <footer className="mt-32 pt-12 border-t border-stone-800 text-center text-stone-400 transition-colors duration-300">
      ...
      <style>{`
        :root {
          --whisky-bg: #0a0705;
          --whisky-surface: rgba(24, 17, 12, 0.92);
          --whisky-panel: rgba(30, 21, 14, 0.90);
          --whisky-body: #d4c5a6;
          --whisky-cream: #e8dcc4;
          --whisky-gold: #d4af37;
